##################################################
# HelloID-Conn-Prov-Target-SDB-Identity-Disable
# PowerShell V2
##################################################
...
try {
    ...
    switch ($action) {
        'DisableAccount' {
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Disabling SDB-Identity account with accountReference: [$($actionContext.References.Account)]"
                $null = Invoke-RestMethod @splatDisableUser
            } else {
                Write-Information "[DryRun] Disable SDB-Identity account with accountReference: [$($actionContext.References.Account)], will be executed during enforcement"
            }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not disable SDB-Identity account for: $($actionContext.Data.DisplayName). Error: $($errorObj.FriendlyMessage)"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    } else {
        $auditMessage = "Could not disable SDB-Identity account for: $($actionContext.Data.DisplayName). Error: $($ex.Exception.Message)"
