#################################################################
# HelloID-Conn-Prov-Target-SDB-Identity-RevokePermission-Group
# PowerShell V2
#################################################################
...
try {
    ...
    switch ($action) {
        'RevokePermission' {
            ...
            Write-Information "Revoking SDB-Identity permission: [$($actionContext.PermissionDisplayName)] - [$($actionContext.References.Permission.Reference)]"
            ...
            if (-not($actionContext.DryRun -eq $true)) {
                Write-Information "Revoke SDB-Identity permission: [$($actionContext.PermissionDisplayName)] - [$($actionContext.References.Permission.Reference)], will be executed during enforcement"
                $null = Invoke-RestMethod @splatRevokeGroupMember
            } else {
                Write-Information "[DryRun] Revoke SDB-Identity permission: [$($actionContext.PermissionDisplayName)] - [$($actionContext.References.Permission.Reference)], will be executed during enforcement"
            }
    ...
    if ($($ex.Exception.GetType().FullName -eq 'Microsoft.PowerShell.Commands.HttpResponseException') -or
        ...
        $auditMessage = "Could not revoke SDB-Identity permission. Error: $($errorObj.FriendlyMessage)"
        Write-Warning "Error at Line '$($errorObj.ScriptLineNumber)': $($errorObj.Line). Error: $($errorObj.ErrorDetails)"
    } else {
        $auditMessage = "Could not revoke SDB-Identity permission. Error: $($_.Exception.Message)"
